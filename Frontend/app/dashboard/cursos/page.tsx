...
import {
  Plus, MoreHorizontal, Pencil, Trash2, Copy, Search,
  BookOpen, User, CheckCircle, XCircle, Loader2, Layers
} from "lucide-react"
import {
  cursosApi, materiaAsigApi,
  type CursoDetalle, type FormularioCurso, type MateriaAsignada,
  type CampoMaterias, type Profesor
} from "@/lib/ciclo2Api"
...
export default function CursosPage() {
  ...
  const [cursoSeleccionado, setCursoSeleccionado] = useState<CursoDetalle | null>(null)
  ...
  const [seleccion, setSeleccion] = useState<{ [id_materia: number]: number }>({}) // materia→profesor
  ...
  const handleSave = async () => {
    if (!editId && (!form.id_grado || !form.paralelo || !form.turno || !form.id_aula || !form.id_profesor)) {
  ...
  const handleDuplicate = async (id: number) => {
    try {
      ...
      load()
    } catch (e: unknown) { toast.error(e instanceof Error ? e.message : "Error al duplicar") }
  }

  const handleDelete = async (id: number) => {
    if (!confirm("¿Desactivar este curso?")) return
    try { await cursosApi.delete(id); toast.success("Curso desactivado"); load() }
    catch (e: unknown) { toast.error(e instanceof Error ? e.message : "Error al desactivar") }
  }

  const handleActivar = async (id: number) => {
    try { await cursosApi.activar(id); toast.success("Curso activado"); load() }
    catch (e: unknown) { toast.error(e instanceof Error ? e.message : "Error al activar") }
  }
  ...
  const openMaterias = async (c: CursoDetalle) => {
    ...
    } catch (e: unknown) { toast.error(e instanceof Error ? e.message : "Error al cargar materias") }
  ...
  const handleCargarPlantilla = async () => {
    ...
    try {
      const r = await materiaAsigApi.cargarPlantilla(cursoSeleccionado.id_curso)
      toast.success(r.message)
      await openMaterias(cursoSeleccionado)
    } catch (e: unknown) { toast.error(e instanceof Error ? e.message : "Error") }
    finally { setGuardandoMaterias(false) }
  }
  ...
  const handleAsignarSeleccionadas = async () => {
    ...
    setGuardandoMaterias(true)
    try {
      const materias = Object.entries(seleccion).map(([id_materia, id_profesor]) => ({
        id_materia: Number(id_materia),
        id_profesor,
      }))
      const r = await materiaAsigApi.asignar(cursoSeleccionado.id_curso, { materias })
      toast.success(r.message)
      setSeleccion({})
      await openMaterias(cursoSeleccionado)
  ...
  const handleCambiarProfesor = async (id_curso_materia: number, id_profesor: number) => {
    try {
  ...
  const filtered = cursos.filter(c =>
    `${c.nombre_nivel} ${c.nombre_grado} ${c.paralelo}`.toLowerCase().includes(search.toLowerCase())
  )
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        {[
          ["Total", cursos.length, ""],
          ["Turno Mañana", cursos.filter(c => c.turno === "Mañana").length, "text-blue-600"],
          ["Turno Tarde", cursos.filter(c => c.turno === "Tarde").length, "text-orange-500"],
        ].map(([l, v, cls]) => (
          ...
          {loading ? (
            ...
            <div className="rounded-md border">
              <Table>
                ...
                <TableBody>
                  {filtered.length === 0 ? (
                    <TableRow><TableCell colSpan={8} className="text-center py-8 text-muted-foreground">Sin cursos</TableCell></TableRow>
                  ) : filtered.map(c => (
                    <TableRow key={c.id_curso}>
                      ...
                      <TableCell>
                        <Badge variant="outline" className={c.turno === "Mañana" ? "text-blue-600" : "text-orange-500"}>
                          {c.turno}
                        </Badge>
                      </TableCell>
                      <TableCell className="text-sm">{c.profesor_titular ?? c.profesor_nombre ?? "—"}</TableCell>
                      <TableCell className="text-sm">{c.total_estudiantes ?? 0}/{c.capacidad_estudiantes ?? "—"}</TableCell>
                      <TableCell>
                        {(c as unknown as { curso_estado?: boolean }).curso_estado !== false
                          ? <Badge variant="default" className="bg-green-500/20 text-green-700 border-green-300">Activo</Badge>
                      ...
                      <TableCell>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem onClick={() => handleDuplicate(c.id_curso)}>
                              <Copy className="h-4 w-4 mr-2" />Duplicar
                            </DropdownMenuItem>
                            {(c as unknown as { curso_estado?: boolean }).curso_estado === false ? (
                              <DropdownMenuItem onClick={() => handleActivar(c.id_curso)} className="text-green-600">
                                <CheckCircle className="h-4 w-4 mr-2" />Activar
                              </DropdownMenuItem>
                            ) : (
                              <DropdownMenuItem onClick={() => handleDelete(c.id_curso)} className="text-destructive">
        ...
        <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Materias — {cursoSeleccionado?.nombre_grado} {cursoSeleccionado?.paralelo}
            </DialogTitle>
          </DialogHeader>
            ...
            <Tabs defaultValue="asignadas">
              ...
              <TabsContent value="asignadas" className="space-y-2 mt-4">
                {materiasAsignadas.length === 0 ? (
                  <p className="text-center text-muted-foreground py-8">No hay materias asignadas aún.</p>
                ) : (
                  <div className="rounded-md border">
                    <Table>
                      <TableHeader>
                      ...
                      <TableBody>
                        {materiasAsignadas.map(m => (
                          <TableRow key={m.id_curso_materia}>
                            <TableCell className="font-medium">{m.nombre_materia}</TableCell>
                            <TableCell>
                              <Select
                                value={String(m.id_profesor)}
                                onValueChange={v => handleCambiarProfesor(m.id_curso_materia, +v)}
                              >
                            ...
                            <TableCell>
                              <Button variant="ghost" size="icon" className="text-destructive h-8 w-8"
                                onClick={() => handleEliminarMateria(m.id_curso_materia)}>
              ...
              <TabsContent value="disponibles" className="space-y-4 mt-4">
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={handleCargarPlantilla} disabled={guardandoMaterias}>
                    {guardandoMaterias ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : null}
                    Cargar plantilla completa
                  </Button>
                  <Button size="sm" onClick={handleAsignarSeleccionadas} disabled={guardandoMaterias || Object.keys(seleccion).length === 0}>
                    {guardandoMaterias ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Plus className="h-4 w-4 mr-1" />}
                    Asignar seleccionadas ({Object.keys(seleccion).length})
                  </Button>
                </div>

                {materiasPorCampo.length === 0 ? (
                  <p className="text-center text-muted-foreground py-8">Todas las materias ya están asignadas.</p>
                  ...
                  <div key={campo.id_campo} className="space-y-2">
                    ...
                    <div className="space-y-1 pl-2">
                      {campo.materias.map(m => {
                        const selected = seleccion[m.id_materia] !== undefined
                        return (
                          <div key={m.id_materia} className={`flex items-center gap-3 rounded-md p-2 border transition-colors ${selected ? "bg-primary/5 border-primary/30" : "border-transparent hover:bg-muted/40"}`}>
                            <input
                              type="checkbox"
                              id={`m-${m.id_materia}`}
                              checked={selected}
                              onChange={e => {
                                if (e.target.checked) {
                                  setSeleccion(prev => ({ ...prev, [m.id_materia]: profesorTitularId || (catalogoProfesores[0]?.id_profesor ?? 0) }))
                                } else {
                                  setSeleccion(prev => { const n = { ...prev }; delete n[m.id_materia]; return n })
                                }
                              }}
                              className="h-4 w-4 accent-primary"
                            />
                            <label htmlFor={`m-${m.id_materia}`} className="flex-1 text-sm cursor-pointer">
                              {m.nombre_materia}
                            </label>
                            {selected && (
                              <Select
                                value={String(seleccion[m.id_materia])}
                                onValueChange={v => setSeleccion(prev => ({ ...prev, [m.id_materia]: +v }))}
                              >
                                ...
                                <SelectContent>
                                  {catalogoProfesores.map(p => (
                                    <SelectItem key={p.id_profesor} value={String(p.id_profesor)}>
                                      <User className="h-3 w-3 mr-1 inline" />{p.nombre} {p.apellido}
                                    </SelectItem>
                                  ))}
                                </SelectContent>
                              </Select>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowMaterias(false)}>Cerrar</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
