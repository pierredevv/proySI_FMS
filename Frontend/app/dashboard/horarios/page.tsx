...
const COLORES: Record<string, string> = {
  ...
  Miércoles: "bg-green-100 text-green-700 border-green-200",
...
export default function HorariosPage() {
  ...
  const openEdit = (b: BloqueHorario) => {
    setEditBloque(b)
    setForm({ id_materia: b.id_materia ?? 0, dia_semana: b.dia_semana, hora_inicio: b.hora_inicio, hora_fin: b.hora_fin, actividad: b.actividad ?? "" })
    setShowDialog(true)
  }
          ...
          {DIAS.map(dia => (
            <Card key={dia} className="flex flex-col">
              <CardHeader className="pb-2 pt-3 px-3">
                <CardTitle className="text-sm font-semibold">{dia}</CardTitle>
              </CardHeader>
              <CardContent className="px-3 pb-3 flex-1 space-y-2">
                {bloquesPorDia(dia).map(b => (
                  <div key={b.id_horario} className={`rounded-md border p-2 text-xs cursor-pointer group relative ${COLORES[dia]}`} onClick={() => openEdit(b)}>
                ...
                <Button variant="ghost" size="sm" className="w-full h-7 text-xs border border-dashed" onClick={() => openAdd(dia)}>
