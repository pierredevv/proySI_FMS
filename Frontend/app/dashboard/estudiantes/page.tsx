...
const ESTADOS = ["activo", "retirado", "egresado"]
...
export default function EstudiantesPage() {
  ...
  const openEdit = (e: Estudiante) => {
    ...
    setForm({ nombre: e.nombre, apellido: e.apellido, ci: e.ci ?? "", fecha_nacimiento: e.fecha_nacimiento ?? "", genero: e.genero, de_traslado: false, institucion_origen: "", observaciones: e.observaciones ?? "" })
    setShowDialog(true)
  }
  ...
  const estadoBadge = (estado: string) => {
    const map: Record<string, string> = { activo: "bg-green-100 text-green-700", retirado: "bg-yellow-100 text-yellow-700", egresado: "bg-gray-100 text-gray-600" }
    return map[estado] ?? ""
  }
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <Select value={estadoFilter} onValueChange={setEstadoFilter}>
              ...
              <SelectContent>
                ...
                {ESTADOS.map(s => <SelectItem key={s} value={s}>{s.charAt(0).toUpperCase() + s.slice(1)}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
        </CardHeader>
