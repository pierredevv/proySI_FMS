...
const put  = <T>(path: string, body: unknown) => apiRequest<T>("PUT", path, body)
...
export const horariosApi = {
  ...
  getByCurso: (idCurso: number) => get<BloqueHorario[]>(`/api/horarios/curso/${idCurso}`),
  // GET /api/horarios/profesor/:id_profesor
  getByProfesor: (idProfesor: number) => get<BloqueHorario[]>(`/api/horarios/profesor/${idProfesor}`),
  // POST /api/horarios
  ...
  update: (id: number, data: Partial<BloqueHorarioPayload>) =>
    put<{ message: string; bloque: BloqueHorario }>(`/api/horarios/${id}`, data),
...
export const estudiantesApi = {
  ...
  create: (data: EstudiantePayload) =>
    post<{ message: string; estudiante: Estudiante; nota_traslado?: string }>(
      "/api/estudiantes", data
    ),
  // PUT /api/estudiantes/:id
  update: (id: number, data: Partial<EstudiantePayload> & { estado?: string }) =>
    put<{ message: string; estudiante: Estudiante; nota_motivo?: string }>(
      `/api/estudiantes/${id}`, data
    ),
...
export const tutoresApi = {
  ...
  search: (q: string) => get<Tutor[]>(`/api/tutores/search?search=${encodeURIComponent(q)}`),
  // POST /api/tutores
  ...
  editarVinculo: (idEstudiante: number, idTutor: number, data: VinculoEditPayload) =>
    put<{ message: string }>(`/api/tutores/vincular/${idEstudiante}/${idTutor}`, data),
  // DELETE /api/tutores/desvincular/:id_estudiante/:id_tutor
  desvincular: (idEstudiante: number, idTutor: number) =>
    del<{ message: string }>(`/api/tutores/desvincular/${idEstudiante}/${idTutor}`),
}
...
export const inscripcionesApi = {
  ...
  inscribir: (data: { id_estudiante: number; id_curso: number; id_gestion: number }) =>
    post<{ message: string; inscripcion: Inscripcion; advertencia?: string }>(
      "/api/inscripciones", data
    ),
  // PUT /api/inscripciones/:id
  retirar: (idInscripcion: number, data: { estado: "retirado" | "trasladado"; motivo: string }) =>
    put<{ message: string; inscripcion: Inscripcion }>(
      `/api/inscripciones/${idInscripcion}`, data
    ),
  // POST /api/inscripciones/traslado/:id
  trasladar: (idInscripcion: number, data: { id_curso_destino: number; motivo: string }) =>
    post<{ message: string; nueva_inscripcion: Inscripcion }>(
      `/api/inscripciones/traslado/${idInscripcion}`, data
    ),
  // POST /api/inscripciones/masiva/csv
  masivaCsv: (data: { id_curso: number; id_gestion: number; csv_text: string }) =>
...
export const expedienteApi = {
  ...
  get: (idEstudiante: number) => get<Expediente>(`/api/expedientes/${idEstudiante}`),
...
export interface FormularioCurso {
  ...
  niveles: { id_nivel: number; nombre_nivel: string; monto_mensualidad?: number }[]
  grados: { id_grado: number; nombre_grado: string; id_nivel: number; nombre_nivel?: string }[]
  aulas: { id_aula: number; numero_aula: string; capacidad_estudiantes: number; descripcion?: string; estado?: string }[]
  profesores: Profesor[]
  turnos?: string[]
}
...
export interface Expediente {
  ...
  tutores: (Tutor & { parentesco: string; autorizado_recoger: boolean; contacto_emergencia: boolean })[]
  inscripciones: Inscripcion[]
...
export interface EstructuraData {
  ...
  aulas: { id_aula: number; nombre_aula: string; capacidad_estudiantes: number }[]
}
