...
const createBloqueHorario = async (req, res) => {
    const { id_curso, id_materia, dia_semana, hora_inicio, hora_fin, actividad } = req.body;
    ...
    try {
        const cursoResult = await pool.query('SELECT id_aula FROM curso WHERE id_curso = $1', [id_curso]);
...
const editarBloqueHorario = async (req, res) => {
    const { id } = req.params;
    const { id_materia, dia_semana, hora_inicio, hora_fin, actividad } = req.body;
    ...
    try {
        ...
        const bloqueActual = await pool.query(
            'SELECT id_curso FROM horario WHERE id_horario = $1', [id]
        );
        ...
        const conflictoAula = await pool.query(`
            ...
            WHERE c.id_aula = $1 AND h.dia_semana = $2
              AND h.hora_inicio < $4 AND h.hora_fin > $3
              AND h.id_horario != $5
        `, [id_aula, diaSemanaFinal, horaInicioFinal, horaFinFinal, id]);
        ...
        // Validar conflicto de profesor si se cambia la materia
        if (id_materia) {
            ...
            if (profMateria.rows.length > 0) {
                ...
                const conflictoProfesor = await pool.query(`
                    ...
                    WHERE cm.id_profesor = $1 AND h.dia_semana = $2
                      AND h.hora_inicio < $4 AND h.hora_fin > $3
                      AND h.id_horario != $5
                `, [id_profesor, diaSemanaFinal, horaInicioFinal, horaFinFinal, id]);
